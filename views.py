...
def new_catagory():
    ...
    if request.method == 'POST':
        ...
        new_catagory = Catagory(name=new_name, user_id = login_session['user_id'])
...
def show_item(catagory_id):
    ...
    if 'username' not in login_session:
        return render_template('public_items.html', catagory=catagory, items=items)
    else:
...
def new_item(catagory_id):
    ...
    if request.method == 'POST':
        ...
        new_item = Item(name=item_name, description=item_desc,
                        catagory_id=catagory.id, user_id = login_session['user_id'])
...
@app.route('/catagory/<int:catagory_id>/<int:item_id>/delete/', methods=['GET', 'POST'])
